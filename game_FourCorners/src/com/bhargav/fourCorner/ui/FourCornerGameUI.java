package com.bhargav.fourCorner.ui;
...
public class FourCornerGameUI implements ActionListener {
	...
	class RemindTask extends TimerTask {
		...
		public void run() {
			if (numWarningBeeps > 0) {
				...
				for (int i = 0; i < FourCornerGameUI.boardSize; i++) {
					for (int j = 0; j < FourCornerGameUI.boardSize; j++) {
	...
	public void createBoardPanel() {
		...
		this.cells = new JButton[FourCornerGameUI.boardSize][FourCornerGameUI.boardSize];
		this.cellsColor = new Color[FourCornerGameUI.boardSize][FourCornerGameUI.boardSize];
		for (int i = 0; i < FourCornerGameUI.boardSize; i++) {
			for (int j = 0; j < FourCornerGameUI.boardSize; j++) {
		...
		this.boardPanel.setLayout(new GridLayout(FourCornerGameUI.boardSize, FourCornerGameUI.boardSize, 1, 1));
	}
	...
	public void createInstructionsPanel() {
		...
		this.instructionsSelectedField.setBackground(FourCornerGameUI.selected);
	...
	public void modifyCellsColor() {
		...
		for (int i = x0; i <= x2; i++) {
			for (int j = y0; j <= y1; j++) {
				this.cells[i][j].setBackground(FourCornerGameUI.colors[random.nextInt(5)]);
				this.cellsColor[i][j] = this.cells[i][j].getBackground();
			}
		}
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (object == this.startButton) {
			...
			if (this.selectedCellsCount != 4) {
				for (i = 0; i < FourCornerGameUI.boardSize; i++) {
					for (j = 0; j < FourCornerGameUI.boardSize; j++) {
				...
				for (i = 0, k = 0; i < FourCornerGameUI.boardSize; i++) {
					for (j = 0; j < FourCornerGameUI.boardSize; j++) {
						if (this.cells[i][j].getBackground().equals(FourCornerGameUI.selected)) {
				...
				if (k < 4) {
					...
					if (isValid) {
						...
						this.resultField.setText("score updated");
				...
				for (int j = 0; j < FourCornerGameUI.boardSize; j++) {
					if (object == this.cells[i][j]) {
						if (this.cells[i][j].getBackground().equals(FourCornerGameUI.selected)) {
							...
							this.cells[i][j].setBackground(FourCornerGameUI.selected);
							this.selectedCellsCount += 1;
						}
					}
				}
